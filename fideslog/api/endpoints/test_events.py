...
from fideslog.api.database.database import get_test_db
from fideslog.api.models.analytics_event import AnalyticsEvent

log = getLogger(__name__)
router = APIRouter(tags=["Test Events"], prefix="/test-events")


@router.post(
    "",
    response_description="The created test event",
    response_model=AnalyticsEvent,
    status_code=status.HTTP_201_CREATED,
)
async def create(
    event: Dict,
    database: Session = Depends(get_test_db),
) -> AnalyticsEvent:
    """
    Create a new test analytics event.
    """
